...
async function scrapeAmazon() {
  ...
  for (let page = 1; page <= totalPages; page++) {
    try {
      ...
      const insertQuery = 'INSERT INTO productspaging (title, price, rating, url) VALUES ?';
...
app.get('/api/productspaging', basicAuth, (req, res) => {
  const page = parseInt(req.query.page) || 1; // Default to page 1 if not provided
  const limit = 10; // Items per page
  const offset = (page - 1) * limit;

  const query = 'SELECT * FROM productspaging LIMIT ? OFFSET ?';
  db.query(query, [limit, offset], (err, results) => {
    ...
    const countQuery = 'SELECT COUNT(*) AS total FROM productspaging';
